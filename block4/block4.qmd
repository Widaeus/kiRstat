...
# Rita första grafen där medelvärdet är 90 mmHg
plot(bt, y, type = "l", xlim = c(-50, 120),
main = "Transformering från mmHg till standardavvikelse",
     xlab = "Blodtryck (mmHg)", ylab = "Densitet")

# Steg A: Subtrahera medelvärdet (µ = 90 mmHg)
lines(bt - 90, y, col = "blue")

# Steg B: Dividera med standardavvikelsen (σ = 10 mmHg)
# för att få standardiserad skala
lines((bt - 90) / 10, y, col = "red")

# Lägg till en legend för att förklara linjerna
legend("topright", legend = c("Original (mmHg)",
...
ggplot(data, aes(x = bt_values, y = densities)) +
  ...
  geom_vline(xintercept = x_bar, color = "red", linetype = "dashed", linewidth = 1.2) +  # Stickprovets medelvikt
  geom_area(data = fill_data, aes(x = fill_values, y = fill_densities), fill = "red", alpha = 0.3) +  # Fyll p-värdesområdet
